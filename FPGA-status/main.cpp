...
{	
	string msg="";
	if(indiv_code & (1<<0)) 
		msg = "		L 	Reset request interface is enabled. Logic in the FPGA fabric can reset the HPS.\n";
	else
		msg = "		L 	Reset request interface is disabled. Logic in the FPGA fabric cannot reset the HPS.\n"; 	
	
	if(indiv_code & (1<<1)) 
		msg =msg+ "		L 	Enables the fpgajtagen bit found in the ctrl register.\n";
	else
		msg =msg+ "		L	Disables the fpgajtagen bit found in the ctrl register.\n"; 	
	
	if(indiv_code & (1<<2)) 
		msg =msg+ "		L 	CONFIG_IO interface is enabled. Execution of the CONFIG_IO instruction \n"+\
				  "            		in the FPGA JTAG TAP controller is supported.\n";
	else
		msg =msg+ "		L	CONFIG_IO interface is disabled. Execution of the CONFIG_IO instruction in the FPGA JTAG TAP controller\n"+ \
				  "           		is unsupported and produces undefined results.\n"; 	

	if(indiv_code & (1<<3)) 
		msg =msg+ "		L 	Boundary-scan interface is enabled. Execution of the boundary-scan instructions \n"+\
				  "            		in the FPGA JTAG TAP controller is supported.\n";
	else
		msg =msg+ "		L 	Boundary-scan interface is disabled. Execution of boundary-scan instructions in the FPGA JTAG TAP\n"+ \
				  "            		controller is unsupported and produces undefined results.\n"; 	

	if(indiv_code & (1<<4)) 
		msg =msg+ "		L	Trace interface is enabled. Other registers in the HPS debug logic must be programmmed to\n" +\
				  " 		 actually send trace data to the FPGA fabric.\n";
	else
		msg =msg+ "		L 	Trace interface is disabled. HPS debug logic cannot send trace data to the FPGA fabric.\n";

	if(indiv_code & (1<<6)) 
		msg =msg+ "		L 	STM event interface is enabled. Logic in the FPGA fabric can trigger STM events.\n";
	else
		msg =msg+ "		L	STM event interface is disabled. Logic in the FPGA fabric cannot trigger STM events.\n";
	
	if(indiv_code & (1<<7)) 
		msg =msg+ "		L 	FPGA Fabric can send triggers.\n";
	else
		msg =msg+ "		L 	FPGA Fabric cannot send triggers.\n";
...
int initMemRegs(void)
{
	...
	systemMangerMap = mmap(NULL,REG_SYSMAN_INDIV_OFFSET+4, PROT_READ, MAP_PRIVATE, posix_fd, \
		REG_SYSMAN_BASE);
...
void deinit(void)
{
	...
	if (systemMangerMap != MAP_FAILED)
	{
		munmap((void*) systemMangerMap, REG_SYSMAN_INDIV_OFFSET+4);
	}
...
int main(int argc, const char* argv[])
{
	...
	else
	{
		...
		cout << "# HPS Info:" <<endl;
		if (is_dualcore==1) cout << "	        Is dual-core (CPU0 and CPU1 both available)."<<endl;
		else				cout << "	        Not dual-core (only CPU0 available)."<<endl;
		
		if (has_can==1) 	cout << "	        CAN0 and CAN1 are available"<<endl;
		else				cout << "	        CAN0 and CAN1 are not available"<<endl;
		...
		if (watchDog0_en==1) 	cout << "	  L     Watchdog 0 enabled and generates a warm reset request"<<endl;
		else					cout << "	  L     Watchdog 0 disabled"<<endl;

		if (watchDog1_en==1) 	cout << "	  L     Watchdog 1 enabled and generates a warm reset request"<<endl;
		else					cout << "	  L     Watchdog 1 disabled"<<endl;

		cout << "#  Interfaces/Signals between the FPGA and HPS:" <<endl;

		if (global_inf_en==1) 	cout << "	   L    Interfaces between FPGA and HPS are not all global disabled"<<endl;
		else					cout << "	   L    [INTERFACE GLOBAL RESET] All interfaces between FPGA and HPS are disabled."<<endl;
